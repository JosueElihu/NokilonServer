...
Public Function InitializeIconList(Optional ByVal IconWidth As Long = 16, _
                                   Optional ByVal IconHeight As Long = 16, Optional ByVal hBitmap As Long, Optional ByVal MaskColor As Long = &HFFFFFFFF)

    '-- Initialize imagelist
    m_hImageList = ImageList_Create(IconWidth, IconHeight, ILC_MASK Or ILC_COLORDDB, 0, 0)
    
    If (MaskColor <> &HFFFFFFFF) Then
        If m_hImageList Then ImageList_AddMasked m_hImageList, hBitmap, MaskColor
    Else
         If m_hImageList Then ImageList_Add m_hImageList, hBitmap, 0
    End If
...
Private Function pvCreate(ByVal SizeGrip As Boolean, _
  ...
  Dim uRct   As RECT2
    ...
    If (m_hStatusBar) Then
        ...
        '-- Update control height
        Call SendMessage(m_hStatusBar, SB_GETRECT, 0, uRct)
        Let UserControl.Height = (uRct.y2 + 1) * Screen.TwipsPerPixelY
        '-- Force bottom align
