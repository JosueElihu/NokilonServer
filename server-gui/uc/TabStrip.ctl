...
Private Sub c_SubClass_WindowProc(bHandled As Boolean, lReturn As Long, hwnd As Long, uMsg As ssc_eMsg, wParam As Long, lParam As Long)
    Dim uNMH        As NMHDR
...
Private Sub UserControl_Show()
    If Ambient.UserMode Then
        ...
        c_SubClass.Subclass UserControl.hwnd
        c_SubClass.AddMsg UserControl.hwnd, WM_NOTIFY, MSG_BEFORE
    End If
End Sub

Private Sub UserControl_Terminate()
    If m_iml Then ImageList_Destroy m_iml
    If hTabs <> 0 Then
        If Not c_SubClass Is Nothing Then
            c_SubClass.UnSubclass UserControl.hwnd
...
Public Sub CreateImageList(Optional ByVal Width As Long = 16, Optional ByVal Height As Long = 16, Optional ByVal hBitmap As Long, Optional ByVal MaskColor As Long = &HFFFFFFFF)
    '-- Initialize imagelist
    m_iml = ImageList_Create(Width, Height, &H20, 0, 0)
    
    If m_iml = 0 Then Exit Sub
    Call SendMessageLong(hTabs, TCM_SETIMAGELIST, 0, m_iml)
    
    If hBitmap = 0 Then Exit Sub
